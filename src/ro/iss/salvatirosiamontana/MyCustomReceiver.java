...
public class MyCustomReceiver extends BroadcastReceiver {
    ...
    public void createNotification(Context context, String content, String url,
            int id, String title) {
        NotificationManager notificationManager = (NotificationManager) context
                .getSystemService(Context.NOTIFICATION_SERVICE);
        Notification notification = new Notification(
                ro.iss.salvatirosiamontana.R.drawable.logo_frunza, content,
                System.currentTimeMillis());
        // Hide the notification after its selected
        notification.flags |= Notification.FLAG_AUTO_CANCEL;
        notification.defaults |= Notification.DEFAULT_SOUND;

        Intent intent = SalvatiRosia.createIntent(context, content, url);
    ...
    public void onReceive(Context arg0, Intent intent) {
        try {
            ...
            SharedPreferences settings = arg0.getSharedPreferences(
                    MainConstants.PUSH_PREFS_NAME, 0);
            int id = settings.getInt(MainConstants.KEY_NOTIFICATION_ID, 0);

            createNotification(arg0, content, url, id++, title);

            SharedPreferences.Editor editor = settings.edit();
            editor.putInt(MainConstants.KEY_NOTIFICATION_ID, id);
            editor.commit();
